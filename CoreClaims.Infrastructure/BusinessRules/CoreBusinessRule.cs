...
namespace CoreClaims.Infrastructure.BusinessRules
{
    public class CoreBusinessRule : ICoreBusinessRule
    {
        ...
        public CoreBusinessRule(
            ...
            IMemberRepository memberRepository, BusinessRulesConfig config)
        {
        ...
        public async Task<ClaimDetail> AssignClaim(ClaimDetail claim)
        {
            ...
            if (claim.TotalAmount < _config.AutoApproveThreshold)
            {
                claim.ClaimStatus = ClaimStatus.Approved;
                claim.Comment = $"[Automatic] Approved: Less than threshold of {_config.AutoApproveThreshold}";
                return claim;
            }
        ...
        public async Task<ClaimDetail> AdjudicateClaim(ClaimDetail claim)
        {
            ...
            if (claim.LastAmount.HasValue)
            {
                ...
                if (difference <= _config.RequireManagerApproval)
                {
                    claim.ClaimStatus = ClaimStatus.Approved;
                    claim.Comment = "[Automatic] Approved: Proposed adjustment below approval threshold";
                    return claim;
                }
            }
