...
$deploymentState = $(az deployment group create -g $resourceGroup --template-file $script --parameters suffix=$suffix --parameters openAiName=$openAiName --parameters openAiDeployment=$openAiCompletionsDeployment -openAiRg $openAiRg --query "properties.provisioningState" -o tsv) 
Pop-Location
Pop-Location
